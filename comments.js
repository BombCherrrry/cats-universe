...
async function initThread(slug, opts) {
  ...
  let sortOrder = "new"; // "new" — новые сверху (по умолчанию), "old" — старые сверху
  let lastRows = [];

  const sortBar = document.createElement("div");
  sortBar.style.cssText = "display:flex; justify-content:flex-end; margin-bottom:10px;";
  sortBar.innerHTML = `
    <button id="sortToggle" style="background:none; border:1px solid var(--border); color:var(--text-faint); border-radius:999px; padding:6px 13px; font-size:0.8rem;">
      Сначала новые ▾
    </button>`;
  ...
  document.getElementById("sortToggle").addEventListener("click", (e) => {
    ...
    e.target.textContent = sortOrder === "new" ? "Сначала новые ▾" : "Сначала старые ▾";
    render(lastRows);
  });
  ...
  function render(rows) {
    ...
    let top = rows.filter(r => !r.parent_id);
    if (sortOrder === "new") top = [...top].reverse();
    const byParent = {};

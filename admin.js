...
function showPanel() { gateView.style.display = "none"; panelView.style.display = "block"; }
...
// Читает список файлов в папке репозитория. Возвращает { "1.webp": sha, ... }
async function ghListDir(path) {
  try {
    const items = await ghRequest(path);
    if (!Array.isArray(items)) return {};
    const map = {};
    items.forEach(it => { map[it.name] = it.sha; });
    return map;
  } catch (e) {
...
// Загружает файл. sha передаётся, если файл уже существует (тогда он перезапишется).
...
document.getElementById("uploadBtn").addEventListener("click", async () => {
  ...
  try {
    const existing = await ghListDir(`comics/${slug}`);
    uploadLog.line(`Загружаю ${files.length} страниц...`);
    for (let i = 0; i < files.length; i++) {
      const name = `${i + 1}.${ext(files[i].name)}`;
      const path = `comics/${slug}/${name}`;
      await ghPutFile(path, await fileToBase64(files[i]), `Страница ${i + 1} — ${title}`, existing[name]);
      pagePaths.push(path);
...
async function deleteComic(slug) {
  ...
  try {
    const { data, sha } = await readComicsJson();
    const matching = data.comics.filter(x => x.slug === slug);
    if (!matching.length) return;
    const title = matching[0].title;

    // собираем все страницы всех совпадающих записей, только локальные файлы
    const paths = new Set();
    matching.forEach(c => comicPages(c).forEach(p => { if (!/^https?:/i.test(p)) paths.add(p); }));

    manageLog.line(`Удаляю файлы страниц (${paths.size})...`);
    ...
    await writeComicsJson(data, sha, `Удалён выпуск: ${title}`);
    manageLog.line("Выпуск удалён ✓ Сайт обновится через 1-2 минуты.", "ok");
    loadComics();
  } catch (err) { manageLog.line("Ошибка: " + err.message, "err"); }
}
...
addPagesInput.addEventListener("change", async () => {
  ...
  try {
    ...
    for (const f of newFiles) {
      n += 1;
      const name = `${n}.${ext(f.name)}`;
      const path = `comics/${slug}/${name}`;
      await ghPutFile(path, await fileToBase64(f), `Страница ${n} — ${c.title}`, existing[name]);
      pages.push(path);
...
// ---------- старт (после объявления всех переменных выше) ----------
